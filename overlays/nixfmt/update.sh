...
new_version=$(curl --silent https://api.github.com/repos/ironm00n/nixfmt/git/refs/heads/master | jq '.object.sha' --raw-output)
new_date=$(curl --silent https://api.github.com/repos/ironm00n/nixfmt/git/commits/"$new_version" | jq '.committer.date' --raw-output)
...
cabal2nix --jailbreak \
  "https://github.com/ironm00n/nixfmt/archive/${new_version}.tar.gz" \
  >> "$derivation_file"
